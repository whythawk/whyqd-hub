      ...
      <div class="space-y-6 bg-white py-6 px-4 sm:p-6">
        ...
        <div class="space-y-1">
          <div class="flex items-center justify-between">
            <p class="text-sm font-medium text-ochre-500 align-middle">
            ...
            <Switch v-model="totpEnabled"
              ...
              <span aria-hidden="true"
                :class="[totpEnabled ? 'bg-ochre-500' : 'bg-gray-200', 'pointer-events-none absolute mx-auto h-4 w-9 rounded-full transition-colors duration-200 ease-in-out']" />
              <span aria-hidden="true"
              ...
              <DialogPanel
                ...
                <div class="sm:flex sm:items-start">
                  <div
                    ...
                    <QrCodeIcon class="h-6 w-6 text-ochre-500" aria-hidden="true" />
                  </div>
                  <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                    <DialogTitle as="h3" class="text-lg font-medium leading-6 text-gray-900">Enable two-factor security
                    </DialogTitle>
                      ...
                      <ul role="list" class="space-y-3">
                        <li class="flex items-start">
                          ...
                          <p class="ml-3 text-sm leading-6 text-gray-600">
                            Download an authenticator app that supports Time-based One-Time Password (TOTP) for your
                            mobile
                            device.
                          </p>
                        </li>
                        <li class="flex items-start">
                          ...
                          <div class="ml-3 text-sm leading-6 text-gray-600">
                            ...
                            <QrcodeVue :value="totpNew.uri" :size="qrSize" level="M" render-as="svg"
                              class="my-2 mx-auto" />
                            <p>
                        ...
                        <li class="flex items-start">
                          ...
                          <div class="ml-3 text-sm leading-6 text-gray-600">
                            ...
                            <Form @submit="enableTOTP" :validation-schema="totpSchema">
                              <div class="space-y-1">
                                <label for="claim" class="block text-sm font-medium text-gray-700 mt-4">6-digit
                              ...
                              <div class="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse">
                                <button type="submit"
                                  class="inline-flex w-full justify-center rounded-md border border-transparent bg-ochre-500 px-4 py-2 text-base font-medium text-white shadow-sm hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-ochre-500 focus:ring-offset-2 sm:ml-3 sm:w-auto sm:text-sm">
                                  Submit
                                </button>
                                <button type="button"
                                  class="mt-3 inline-flex w-full justify-center rounded-md border border-gray-300 bg-white px-4 py-2 text-base font-medium text-gray-700 shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-sienna-500 focus:ring-offset-2 sm:mt-0 sm:w-auto sm:text-sm"
...
async function submit(values: any, { resetForm }) {
  ...
  if ((!authStore.profile.password && !values.original) ||
    ...
    if (totpEnabled.value !== authStore.profile.totp && totpEnabled.value) {
      await authStore.authTokens.refreshTokens()
      const { data: response } = await apiAuth.requestNewTOTP(authStore.authTokens.token)
      if (response.value) {
