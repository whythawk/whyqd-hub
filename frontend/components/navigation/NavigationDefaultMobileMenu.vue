    ...
    <Menu as="div" class="z-40 fixed md:hidden inset-x-0 bottom-0 w-full bg-white">
      <div class="max-w-full mx-auto">
        <nav class="relative grid grid-cols-5 gap-4 justify-center py-1" aria-label="Global">
          <NuxtLink v-for="(item, i) in baseNavigation" :key="`basenav-${i}`" :to="item.to"
            :class="[item.name === appSettings.current.pageName ? 'bg-gray-50 text-ochre-600' : 'text-gray-700 hover:text-ochre-600 hover:bg-gray-50', 'group flex-col inline-flex items-center gap-x-3 rounded-md p-2 text-sm font-semibold']">
      ...
      <transition enter-active-class="transition ease-out duration-200" enter-from-class="transform opacity-0 scale-95"
        ...
        <MenuItems
          ...
          <NuxtLink :to="item.to"
            :class="[item.name === appSettings.current.pageName || active ? 'bg-gray-50 text-ochre-600' : 'text-gray-700 hover:text-ochre-600 hover:bg-gray-50', 'group flex gap-x-3 rounded-md p-2 text-sm leading-6 font-semibold']">
            <component :is="item.icon"
